...
ROTATING_PROXY = "http://hknlieja-rotate:ox0xi8w7ukfj@p.webshare.io:80"  # Replace with your rotating proxy endpoint
...
def get_payment_intent(session, amount="10"):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.error(f"Attempt {attempt + 1}/{max_retries} - Network error: {str(e)} - Status: {status_code} - Response: {response_text}")
...
def main():
    ...
    if not ROTATING_PROXY:
        print(Fore.RED + "Error: No rotating proxy configured. Please set ROTATING_PROXY.")
        return
    print(Fore.CYAN + f"Bot is running with rotating proxy: {ROTATING_PROXY.split('@')[1] if '@' in ROTATING_PROXY else ROTATING_PROXY}")
