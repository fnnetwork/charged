...
ROTATING_PROXY = """http://PNFTzjJC2spufSEy:VrYCdvs0Fif6OrD8@geo.g-w.info:10080
...
http://4wEgSx2BQ54N33mW:qRkHr6ItRMXQPv9l@geo.g-w.info:10080""".split("\n")  # Replace with your residential rotating proxy

def create_session(proxy=None):
    """Create a new session with headers and cookies matching Chut2.py"""
...
async def chk_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        session = create_session(ROTATING_PROXY)
...
def main():
    ...
    if not ROTATING_PROXY:
        print(Fore.RED + "Warning: No proxy configured. This may fail on data center IPs like DigitalOcean.")
    else:
        print(Fore.CYAN + f"Bot is running with proxy: {ROTATING_PROXY.split('@')[1] if '@' in ROTATING_PROXY else ROTATING_PROXY}")
