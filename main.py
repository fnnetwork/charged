...
def get_payment_intent(session, amount="10"):
    ...
    # Initialize cookies
    try:
        session.get(donation_url, timeout=15)
        logger.debug(f"Cookies set: {session.cookies.get_dict()}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.error(f"Attempt {attempt + 1}/{max_retries} - Network error: {str(e)} - Response: {e.response.text if e.response else 'No response'}")
